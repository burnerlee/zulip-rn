...
export default function SubscriptionsScreen(props: Props): Node {
  ...
  const sections = useMemo(() => {
    const sortedSubscriptions = subscriptions
      .slice()
      .sort((a, b) => {
        // Sort by muted status first, then by name
        if (a.in_home_view !== b.in_home_view) {
          return a.in_home_view ? -1 : 1; // Non-muted streams first
        }
        return caseInsensitiveCompareFunc(a.name, b.name);
      });
    return [
